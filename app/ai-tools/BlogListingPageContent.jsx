//updated code of blog listing page for the sufian website
...
export default function BlogListingPageContent({
  ...
  // Derive dynamic subcategory filter chips from server data
  ...
  // Filter & Sort Selection Handlers
  ...
  // Build dynamic GROQ query when filter or sort is active
  const customQuery = useMemo(() => {
    const isDefaultSort = sortBy === "publishedAt desc";
    const isAllCategory = selectedCategory === "all";

    // Default: no custom query needed — ReusableCachedAllBlogs uses its built-in query
    if (isAllCategory && isDefaultSort) return null;

    const filterClause = !isAllCategory
      ? ` && references("${selectedCategory}")`
      : "";

    return `*[_type=="${schemaType}"${filterClause}] | order(${sortBy}) {
      ...
      "subcategories": subcategories[]->{_id, title, "slug": slug.current}
    }`;
  }, [selectedCategory, sortBy, schemaType]);
  ...
  return (
    <PageCacheProvider pageType={pageSlugPrefix} pageId="main">
      ...
      {/* Premium Header Context Section */}
      ...
      {/* Premium Interactive Control & Optimization Panel */}
        ...
        <div className="idc-control-panel">
          <div className="idc-search-row">
            <div className="idc-search-box">
              ...
              <input
                type="text"
                placeholder={`Search ${pageTitle.toLowerCase()}, frameworks, or keywords...`}
          ...
          {/* Expanded Dynamic Filter and Category Matrix Rows */}
          <div className={`idc-filter-row ${showFilters ? "idc-filter-row--open" : ""}`}>
            <div className="idc-chip-row" aria-label="Insight filter segments">
      ...
      {/* Directory Metrics Index Indicator */}
      ...
      {/* Grid Content Render Tree (Search Layer vs Standard Paginated Layer) */}
      {searchHook.isSearchActive ? (
        ...
        <section className="mb-16" aria-label="Paginated content architecture">
          <ReusableCachedAllBlogs
            ...
            initialPageData={!customQuery && currentPage === 1 ? finalInitialData?.firstPageBlogs : undefined}
            initialTotalCount={!customQuery ? (finalInitialData?.totalCount ?? finalInitialData?.totalCountBlogs ?? 0) : undefined}
            customQuery={customQuery}
            isSearchMode={!!customQuery}
          />
        </section>
      )}

      {/* Premium Controlled Pagination Unit */}
      {!searchHook.isSearchActive && (
      ...
      {/* Embedded High Fidelity Aesthetic Core Style Engine */}
      <style jsx global>{`
        .idc-control-section {
          margin-bottom: 24px;
        }
